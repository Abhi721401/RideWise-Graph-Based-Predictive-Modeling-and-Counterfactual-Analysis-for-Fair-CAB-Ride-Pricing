...
# ── PRESETS
PRESETS = {
    "JFK → Times Sq":       (40.6413, -73.7781, 40.7580, -73.9855),
    "Times Sq → Brooklyn":  (40.7580, -73.9855, 40.6782, -73.9442),
    "Midtown → LGA":        (40.7549, -73.9840, 40.7769, -73.8740),
    "Wall St → Central Pk": (40.7074, -74.0113, 40.7812, -73.9665),
}
...
with left:
    ...
    for i, (name, coords) in enumerate(PRESETS.items()):
        with pcols[i % 2]:
    ...
    # Set defaults from preset or session state
    if selected_preset:
        st.session_state['pu_lat'] = selected_preset[0]
        st.session_state['pu_lon'] = selected_preset[1]
        st.session_state['do_lat'] = selected_preset[2]
        st.session_state['do_lon'] = selected_preset[3]

    pu_lat_def = st.session_state.get('pu_lat', 40.7580)
    pu_lon_def = st.session_state.get('pu_lon', -73.9855)
    do_lat_def = st.session_state.get('do_lat', 40.6413)
    do_lon_def = st.session_state.get('do_lon', -73.7781)

    # Pickup
    st.markdown('<span class="sec-label" style="margin-top:8px">Pickup</span>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    pu_lat = c1.number_input("Latitude",  value=pu_lat_def, format="%.4f", step=0.001, key="pu_lat_in")
    pu_lon = c2.number_input("Longitude", value=pu_lon_def, format="%.4f", step=0.001, key="pu_lon_in")

    # Dropoff
    st.markdown('<span class="sec-label" style="margin-top:8px">Dropoff</span>', unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    do_lat = c3.number_input("Latitude",  value=do_lat_def, format="%.4f", step=0.001, key="do_lat_in")
    do_lon = c4.number_input("Longitude", value=do_lon_def, format="%.4f", step=0.001, key="do_lon_in")
...
with right:
    if run_btn or selected_preset:
        ...
        with tab_map:
            fig_map = plot_route_on_map(
                art['zone_stats'], art['zone_df'],
                pu_zone, do_zone,
...
# st.markdown("""
# <div class="topbar">
#     <div class="topbar-logo">Ride<span>Wise</span></div>
#     <div class="topbar-badge">GNN · Fairness · Counterfactuals</div>
# </div>
# """, unsafe_allow_html=True)
...
#     passengers = c7.selectbox("Pax", [1,2,3,4,5,6], index=0)
